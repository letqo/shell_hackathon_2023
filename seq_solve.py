...
def solve_batch(batch_num, used_depots, num_dep_satur):
    ...
    global batch_increment,biomass_data_global, distance_matrix_global, \
           total_objective, depots_holding
    ...
    biomass_data = biomass_data_global.iloc[batch_num:batch_num+batch_increment+surplus,:4]

    distance_matrix = distance_matrix_global.iloc[batch_num:batch_num+batch_increment+surplus,\
                                                  1+batch_num:1+batch_num+batch_increment+surplus]
    ...
    depots_indices = list(range(num_harvesting_sites))
    ...
    a = 0.001
    c = 1
    cap_depot = dict([(j,20000) for j in range(num_harvesting_sites)])
    ...
    cost_transport = lpSum(distance_matrix.iloc[i, j] * biomass_demand_supply[i, j] for i in range(num_harvesting_sites) \
                    ...
                    +lpSum(distance_matrix_global.iloc[i+batch_num,j]*biomass_demand_supply[i,j] for i in range(num_harvesting_sites)\
                                                                                        for j in previous_depots)
    cost_underutilization = lpSum(depot[j]*cap_depot[j] for j in depots_indices) - \
                          lpSum(biomass_demand_supply[i, j] for i in range(num_harvesting_sites) \
    ...
    for j in depots_indices:
        model += lpSum(biomass_demand_supply[i, j] for i in range(num_harvesting_sites)) <= depot[j]*cap_depot[j]
    ...
    total_processed_biomass = lpSum(biomass_demand_supply[i, j] for i in range(num_harvesting_sites) for j in depots_indices)
    ...
    for v in model.variables():
        ...
        if v.varValue!=0 and 'depot' not in v.name:
            ...
            source = batch_num+int(splitted_name[-2].lstrip('(').rstrip(','))
            sink = int(splitted_name[-1].rstrip(')'))
            if sink in previous_depots:
                sink+=batch_num
        ...
        if 'depot' in v.name:
            ...
            if v.varValue <= 1.0 and v.varValue >= 0.8:
                ...
                holding = lpSum(biomass_demand_supply[i, index] for i in range(num_harvesting_sites))
                satur = 20000 - marge
                if index in previous_depots:
                    ...
                    splitted_name = (v.name).split("_")
                    index += batch_num # Update the index to the global dataset format
...
biomass_data_global = pd.read_csv('Biomass_History.csv')
distance_matrix_global = pd.read_csv('hackerank/dataset/Distance_Matrix.csv')
used_depots = dict()
num_dep_satur = 0
